<section class="hero">
    <div class="container">
      ...
      <div class="hero-content pb-5 text-center">
        <h1 class="hero-heading"><?= $typeObject->getName() ?></h1>
        <div class="row">
      ...
      <div class="row">
        ...
        <div class="product col-xl-3 col-lg-4 col-sm-6">
          <div class="product-image">
            <a href="<?= $router->generate('product', ['id' => $currentProduct->getId()]) ?>" class="product-hover-overlay-link">
              <img src="<?= $currentProduct->getPicture() ?>" alt="product" class="img-fluid">
            </a>
          </div>
          <div class="product-action-buttons">
            <a href="#" class="btn btn-outline-dark btn-product-left"><i class="fa fa-shopping-cart"></i></a>
            <a href="<?= $router->generate('product', ['id' => $currentProduct->getId()]) ?>" class="btn btn-dark btn-buy"><i class="fa-search fa"></i><span class="btn-buy-label ml-2">Voir</span></a>
          </div>
          <div class="py-2">
            <p class="text-muted text-sm mb-1"> <?= $allTypes[$currentProduct->getType_Id()] ?> </p>
            <h3 class="h6 text-uppercase mb-1"><a href="<?= $router->generate('product', ['id' => $currentProduct->getId()]) ?>" class="text-dark"><?= $currentProduct->getName()?></a></h3><span class="text-muted"><?= $currentProduct->getPrice()?> €</span>
          </div>
        </div>
        <?php endforeach ?>
        <!-- /product-->

        <?php dd($currentProduct); ?>
